-- Thanks for http://codinginfinity.me for this module
...
generateSitemap config = do
    ids <- getMatches "**"
    urls <- filter (sitemapFilter config . fst) . filter extFilter . catMaybes <$> mapM routeWithMod ids
